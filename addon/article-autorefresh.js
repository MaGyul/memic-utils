...
function toggleAutoRefresh() {
    ...
    addonStorage.set("autoRefreshEnabled", autoRefreshEnabled);

    if (autoRefreshEnabled) {
        intervalId = setInterval(clickRefreshButton, 5000);
        logger.log("자동 새로고침 시작");
    } else {
        clearInterval(intervalId);
        logger.log("자동 새로고침 중지");
    }
    updateToggleButton();
}
...
function removeToggleButton(retryCount = 0) {
    if (
        !document.getElementById("auto-refresh-toggle") ||
        !window.location.href.includes("?type=community")
    ) return;
    ...
    if (!header) {
        if (retryCount < 20) {
            setTimeout(() => removeToggleButton(retryCount + 1), 500);
        }
        return;
    }

    header.removeChild(document.getElementById("auto-refresh-toggle"));
}
